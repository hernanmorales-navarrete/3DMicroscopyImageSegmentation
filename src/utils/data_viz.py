...
def visualize_reconstructed_images(
    ...
    def update(z):
        ...
        img = unpatchify(img_reshaped, padded_image_sizes[image_index])[
            : nonpadded_image_sizes[image_index][0],
            : nonpadded_image_sizes[image_index][1],
            : nonpadded_image_sizes[image_index][2],
        ]
        mask = unpatchify(mask_reshaped, padded_image_sizes[image_index])[
            : nonpadded_image_sizes[image_index][0],
            : nonpadded_image_sizes[image_index][1],
            : nonpadded_image_sizes[image_index][2],
        ]
        ...
        for i in range(num_models):
            row = (i + 2) // 3
            col = (i + 2) % 3
            prediction_patches_matrix = create_matrix_images_as_rows_patches_as_cols(
                predictions[i], reshaped_patches_arr_sizes
            )
            pred_reshaped = prediction_patches_matrix[image_index].reshape(
                nonreshaped_patches_arr_sizes[image_index]
            )
            pred = unpatchify(pred_reshaped, padded_image_sizes[image_index])[
                : nonpadded_image_sizes[image_index][0],
                : nonpadded_image_sizes[image_index][1],
                : nonpadded_image_sizes[image_index][2],
            ]
