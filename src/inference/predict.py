...
class Prediction:
    ...
    def __init__(self, patch_dir: Path, images_dir: Path, output_dir: Path, dataset_name: Path, models_dir: Path):
        ...
        #Get deep learning models from dir and dataset_name
        self.models_list = get_deep_learning_models_from_dir(models_dir, AVAILABLE_MODELS, AVAILABLE_AUGMENTATIONS)
        ...
        overwrite_and_create_directory(self.directory_for_image_level_predictions)
        overwrite_and_create_directory(self.directory_for_patch_level_predictions)
    
    def predict(self):
        for subdir in self.subdirs_corresponding_to_image_names:
            # Obtain image's patches filenames and sort them by ID
            # IMPORTANT: THE IMAGES SHOULD BE SORTED! 
            image_patches_absolute_paths = sorted(subdir.glob("*"), key=lambda filename: extract_patch_info_from_path(filename).patch_id)
            ...
            for method in CLASSICAL_METHODS: 
                apply_classical_thresholding_and_save_masks_for_array_of_filenames(image_patches_absolute_paths, self.directory_for_patch_level_predictions, subdir.name, method, MAX_WORKERS)
                apply_classical_thresholding_and_save_masks_for_array_of_filenames([complete_image_filename], self.directory_for_image_level_predictions, subdir.name ,method, MAX_WORKERS)
            ...
            for model_name, augmentation, best_model_path in self.models_list: 
                apply_deep_learning_method_to_array_of_filenames(image_patches_absolute_paths, self.directory_for_patch_level_predictions, self.directory_for_image_level_predictions, subdir.name , model_name, augmentation, best_model_path, BATCH_SIZE, PATCH_SIZE, MAX_WORKERS)