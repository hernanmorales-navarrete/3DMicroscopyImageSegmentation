...
from src.data_loader.reconstruct_dataset import (
    create_dataset_inference,
    create_dataset_prediction,
    create_matrix_images_as_rows_patches_as_cols,
)

from src.utils.data_viz import (
    visualize_patches_3D_in_2D,
...
def inference(input_dir, model_path, model_name, patch_shape, patch_step, batch_size, out_dir):
    ...
    save_masks_inference(
        predictions,
        reconstruction_info[3],
        reconstruction_info[2],
        reconstruction_info[1],
        reconstruction_info[0],
        [model_name],
        all_images_names,
        out_dir,
    )
...
def inference_evaluation(
    ...
    interact(
        ...
        model_names=fixed(model_names),
        patch_idx=patch_slider,
        z=z_slider,
    )

    img_patches_matrix = create_matrix_images_as_rows_patches_as_cols(
        dataset[0], reconstruction_info[3]
    )
    mask_patches_matrix = create_matrix_images_as_rows_patches_as_cols(
        dataset[1], reconstruction_info[3]
    )
    ...
    interact(
        visualize_reconstructed_images,
        images=fixed(img_patches_matrix),
        masks=fixed(mask_patches_matrix),
        predictions=fixed(predictions),
        model_names=fixed(model_names),
        nonreshaped_patches_arr_sizes=fixed(reconstruction_info[2]),
        reshaped_patches_arr_sizes=fixed(reconstruction_info[3]),
        nonpadded_image_sizes=fixed(reconstruction_info[0]),
        padded_image_sizes=fixed(reconstruction_info[1]),
